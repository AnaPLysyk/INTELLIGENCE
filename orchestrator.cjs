...
function build() {
  ...
  const result = spawnSync(process.execPath, [tsc, '-p', 'tsconfig.cucumber.json'], { cwd: root, stdio: 'inherit', shell: false, env: process.env });
...
const suiteName = explicit || process.env.AUTOMATION_SUITE?.trim() || (scheduled ? suiteFromSchedule(process.env.GITHUB_EVENT_SCHEDULE?.trim()) : plan.defaultSuite);
...
const tags = ticket ? `@${ticket.toLowerCase()}` : historicalRelease ? tagExpressionFromTickets(historicalTickets) : suite.grep;
const selectedRelease = historicalRelease || plan.release;

if (args.includes('--show-plan')) {
  console.log(JSON.stringify({ project: plan.project, release: selectedRelease, ...suite, selectedSuite: suiteName, selectedTicket: ticket, selectedHistoricalRelease: historicalRelease, selectedHistoricalTickets: historicalTickets, selectedTags: tags }, null, 2));
  process.exit(0);
}
...
const result = spawnSync(process.execPath, cucumberArgs, { cwd: root, stdio: 'inherit', shell: false, env: process.env });
if (result.error) fail(result.error.message);

if (!args.includes('--list')) {
  const qa = spawnSync(process.execPath, [path.join(root, 'scripts', 'qa-result.cjs')], { cwd: root, stdio: 'inherit', shell: false, env: process.env });
