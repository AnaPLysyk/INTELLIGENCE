...
async function lerBody(response: Awaited<ReturnType<APIRequestContext['get']>>): Promise<unknown> {
...
export async function autenticarSmart(request: APIRequestContext): Promise<string> {
  const response = await request.post(`${baseUrlSmart()}/api/tokens`, {
...
export async function consultarProcessoSmart(
  ...
  const response = await request.get(`${baseUrlSmart()}/api/processos/${processId}`, {
