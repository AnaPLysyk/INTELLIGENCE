...
import { autenticarGbds, consultarTransacaoGbds } from '../../api/integracao/gbds/gbds.client';
import { consultarProcessoSmart, autenticarSmart } from '../../api/integracao/smart/smart.client';
import { consultarSmart } from '../../bd/integracao/smart/smart.repository';
import {
  ...
  extrairItens,
  payloadDaMassa,
} from '../../api/intelligence/intelligence.client';
import { caminhoArquivoMassa, type ArquivoMassaBusca, type EntradaMassaBusca, type IdentidadeEsperada } from '../../../massas/dados/intelligence.busca.massa';
...
export async function gerarMassaDeBuscaComDadosDoSmart(request: APIRequestContext): Promise<ArquivoMassaBusca> {
  ...
  if (processos.length === 0) {
    throw new Error('BLOQUEADO: nenhum processo SMART com TGUID e PGUID foi encontrado.');
  }
  ...
  for (let inicio = 0; inicio < processos.length; inicio += concorrencia) {
    ...
    const resultados = await Promise.all(lote.map(async (processo) => {
      ...
      if (!encontrar(campos, 'EXTERNAL.ID') || !encontrar(campos, 'cib')) {
        const transacaoGbds = await consultarTransacaoGbds(tokenGbds, String(processo.Tguid));
        coletarCampos(transacaoGbds, campos);
      }
      return { processo, campos };
    }));

    const candidatasPesquisaveis: Array<{ tipo: string; item: EntradaMassaBusca }> = [];
    for (const { processo, campos } of resultados) {
      ...
      if (!buscas.cpf && esperado.cpf) candidatasPesquisaveis.push({ tipo: 'cpf', item: entrada('cpf', esperado.cpf, esperado, 'KEY', 'SMART API') });
      if (!buscas.name && esperado.nome) candidatasPesquisaveis.push({ tipo: 'name', item: entrada('name', esperado.nome, esperado, 'BIOGRAPHIC', 'SMART API') });
      if (!buscas.birthdate && esperado.dataNascimento) candidatasPesquisaveis.push({ tipo: 'birthdate', item: entrada('birthdate', esperado.dataNascimento, esperado, 'BIOGRAPHIC', 'SMART API') });
      if (!buscas['EXTERNAL.ID'] && esperado.externalId) candidatasPesquisaveis.push({ tipo: 'EXTERNAL.ID', item: entrada('EXTERNAL.ID', esperado.externalId, esperado, 'EXTERNAL_ID', 'GBDS API') });
      const cib = encontrar(campos, 'cib');
      if (!buscas.cib && cib) candidatasPesquisaveis.push({ tipo: 'cib', item: entrada('cib', cib, esperado, 'KEY', 'GBDS API') });
    }

    const validacoes = await Promise.all(candidatasPesquisaveis.map(async ({ tipo, item }) => ({
      tipo,
      item,
      disponivel: await estaPesquisavelNoIntelligence(request, item, sessionGuid),
    })));
    for (const validacao of validacoes) {
      if (validacao.disponivel) buscas[validacao.tipo] ??= validacao.item;
    }
